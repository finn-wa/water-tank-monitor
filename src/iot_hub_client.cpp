...
/*
 * The new version of AzureIoTHub library change the AzureIoTHubClient
 * signature. As a temporary solution, we will test the definition of
 * AzureIoTHubVersion, which is only defined in the new AzureIoTHub library
 * version. Once we totally deprecate the last version, we can take the #ifdef
 * out.
 *
...
void sendMessage(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle, char *buffer,
  ...
  IOTHUB_MESSAGE_HANDLE messageHandle = IoTHubMessage_CreateFromByteArray(
      (const unsigned char *)buffer, strlen(buffer));
  if (messageHandle == NULL) {
    Serial.println("Unable to create a new IoTHubMessage.");
  } else {
    MAP_HANDLE properties = IoTHubMessage_Properties(messageHandle);
    Map_Add(properties, "temperatureAlert",
